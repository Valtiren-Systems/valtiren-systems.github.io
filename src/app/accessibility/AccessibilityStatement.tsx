...
// same "01, 03, 05..." left / "02, 04, 06..." right pattern as Terms/Privacy.
...
export default function AccessibilityStatement() {
  const [leftColumn, rightColumn] = splitColumns(ACCESSIBILITY_SECTIONS);
        ...
        >
          {[leftColumn, rightColumn].map((column, colIndex) => (
            <ul key={colIndex} className="space-y-3">
              {column.map((section) => {
                const globalIndex = ACCESSIBILITY_SECTIONS.indexOf(section);
                return (
                  <li key={section.id}>
                    <a
                      href={`#${section.id}`}
          ...
          {ACCESSIBILITY_SECTIONS.map((section, index) => (
            ...
            >
              ...
              {section.body.map((paragraph, i) => (
                <p key={i} className="mb-3 leading-relaxed text-slate-600 last:mb-0">
                  {paragraph}
                </p>
              ))}
            </section>
